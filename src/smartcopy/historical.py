...
Historical BACKFILL ``first_observed_time`` values are ingestion provenance. They are
parsed only because they are part of ``WalletActivity`` and are never used analytically.
"""
...
FROZEN_START = datetime(2026, 8, 25, tzinfo=timezone.utc)
...
class HistoricalEvidenceError(ValueError):
    """Historical evidence cannot satisfy the frozen Stage 2H contract."""
...
def analyze_paired_legs(
    activities: Iterable[WalletActivity], *, start: datetime, end: datetime
    ...
    for item in activities:
        ...
        if item.activity_type.upper() != "TRADE":
            continue
        family = classify_market(title=item.title, slug=item.slug, event_slug=item.event_slug).family
        if family not in _TARGET_FAMILIES or _target_symbol(item) is None:
            continue
    ...
    for condition_id, rows in scoped.items():
        outcomes = {_canonical_outcome(row.outcome) for row in rows}
        if None in outcomes or not outcomes.issubset({"Up", "Down"}):
...
def load_normalized_activity_jsonl(path: str | Path) -> tuple[WalletActivity, ...]:
    ...
    with Path(path).open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                row = json.loads(line)
                if "observation_mode" not in row:
                    raise HistoricalEvidenceError("observation_mode is required")
                items.append(
                    WalletActivity(
                        ...
                        observation_mode=ObservationMode(row["observation_mode"]),
                        raw={},
                    )
                )
...
def write_stage2h_artifacts(
    *,
    ...
    wallets = {item.proxy_wallet for item in activities}
    if wallets != {FROZEN_WALLET}:
        raise HistoricalEvidenceError(f"frozen wallet mismatch: {sorted(wallets)}")
...
def _market_record(condition_id: str, rows: list[WalletActivity]) -> PairedMarketRecord:
    ...
    paired_fraction = 2 * matched / total_buy_size if total_buy_size > 0 else None

    pair_sum = margin = matched_cost = None
    if up.vwap_price is not None and down.vwap_price is not None:
        pair_sum = up.vwap_price + down.vwap_price
        margin = 1 - pair_sum
        matched_cost = matched * pair_sum
    ...
    times = [row.source_event_time for row in rows]
    return PairedMarketRecord(
        ...
        pair_vwap_sum=pair_sum,
        gross_pair_margin_per_unit=margin,
        matched_average_cost=matched_cost,
        first_leg_gap_seconds=gap,
        first_leg_order=order,
        market_activity_span_seconds=(max(times) - min(times)).total_seconds() if times else 0.0,
    )
...
def _leg_stats(rows: list[WalletActivity]) -> LegStats:
    ...
    return LegStats(
        ...
        vwap_price=(sum(float(row.price) * row.size for row in priced) / priced_size) if priced_size else None,
        first_source_event_time=min(times) if times else None,
        last_source_event_time=max(times) if times else None,
    )
...
def _target_symbol(item: WalletActivity) -> str | None:
    ...
    btc, eth = bool(_BTC.search(text)), bool(_ETH.search(text))
    if btc == eth:
        return None
...
def _market_to_dict(record: PairedMarketRecord) -> dict[str, Any]:
    ...
    for name in ("up", "down"):
        leg = getattr(record, name)
        payload[name]["first_source_event_time"] = _iso(leg.first_source_event_time)
        payload[name]["last_source_event_time"] = _iso(leg.last_source_event_time)
    return payload
